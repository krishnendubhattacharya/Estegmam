...
function chandle(event,d,h) {
  ...
  if (t.id.indexOf('_')==-1) {
    if (auth_write==false) {
      ...
      return;}
    if (grid) {hideDialog('reservation');return}
    var hour = (h>=0) ? 60*Number(t.rowIndex)/interval+h*60 : 720 // to do: better default than 12
    if (res.length>0) {tn=dir==0 ? res[Math.floor((event.clientX-$(t).cumulativeOffset()[0])*res.length/t.offsetWidth)] : tn=res[Math.floor(Math.max(Event.pointerY(event)-$(t).cumulativeOffset()[1]-16,0)*res.length/(t.offsetHeight-15))]} else tn=0
    newres(event,(capacity ? 'slot' : 'reservation' ),x,hour/60,hour%60,tn)}
  else {
    var id= t.id.substring(1,t.id.indexOf('_'))
    if (t.id.substr(0,1)=='e') {
      if (auth_write==false) {displayatmouse("reservation",event);return;}
      ...
      if (capacity) viewslot(event,id); else editres(event,id)}
  }
}
