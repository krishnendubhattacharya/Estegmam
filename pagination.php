...
class PS_Pagination {
	...
	function PS_Pagination($sql, $rows_per_page = 10, $links_per_page = 5) {
...
//		}
	...
	function renderFirst($tag='&lt;&lt; ') {
		if($this->page == 1) {
			return "<font face='Arial' size='2'>" . $tag . "</font>";
		}
		else {
			return '<a class="style2" href="'.$this->php_self.'?page=1">'.$tag.'</a>';
		}
	}
	...
	function renderLast($tag=' &gt;&gt;') {
		if($this->page == $this->max_pages) {
			...
			return '<a class="style2" href="'.$this->php_self.'?page='.$this->max_pages.'">'.$tag.'</a>';
		}
	}
	...
	function renderNext($tag=' Next') {
		if($this->page < $this->max_pages) {
			return ' <a class="style2" href="'.$this->php_self.'?page='.($this->page+1).'">'.$tag.'</a>';
		}
		else {
			return "<font face='Arial' size='2'>" . $tag . "</font>";
		}
	...
	function renderNav() {
		...
		for( $i=$start ; $i<$end ; $i++) {
			if($i == $this->page) {
				$links .= " <font face='Arial' size='2'><b><u>$i</u></b></font> ";
			}
			else {
				$links .= ' <a class="style2" href="'.$this->php_self.'?page=' .$i . '">' . $i . '</a> ';
			}
		}
