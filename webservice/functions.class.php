...
//http://team3.nationalitsolution.co.in/estejmam/webservice/service.php?auth=fcea920f7412b5da7be0cf42b8c93759&action=listProperties
	...
	$per_page_item = isset($_REQUEST['per_page_item']) ? $_REQUEST['per_page_item'] : '10';
	
	$getSQL="select * from `estejmam_main_property` WHERE `name`<>'' order by id DESC";
	if($page_number!=''){
	$getSQL=$getSQL." limit ".(($page_number-1)*$per_page_item).", $per_page_item";
	}
	
	/*	echo $getSQL;
	echo '<br>';*/
	$property = mysql_query($getSQL);
	
	$num = mysql_num_rows($property);
	if ($num > 0) {
	while ($propertyDetails = mysql_fetch_array($property)) {
	
	$currentPostID=$propertyDetails['id'];
	
	unset($postImgaes);
	
	$propertyImages = mysql_query("select * from `estejmam_image` where `prop_id`='" . $currentPostID . "'");	
	while($getPropertyImages=mysql_fetch_array($propertyImages))
	{
	$postImgaes[]= array(
	"image"=> SITE_URL1."upload/property/".$getPropertyImages['image'],
	);
	...
	$userDetails = mysql_fetch_array(mysql_query("SELECT * FROM `estejmam_user` WHERE `id`='" . $propertyDetails['user_id'] . "'"));
	
	if ($userDetails['image'] != '') {
	$user_image = SITE_URL1 . "upload/userimages/" . $userDetails['image'];
	} else {
	$user_image = '';
	}
	
	if(count($postImgaes)>0){
	...
	"street_addr" =>$propertyDetails['street_addr'],
	"zipcode" => $propertyDetails['zipcode'],
	"price" => $propertyDetails['price'],
	"currency" => $propertyDetails['currency'],
	"lat" => $propertyDetails['lat'],
	"lng" => $propertyDetails['lng'],
	"posted_by" => $userDetails['fname'],
	"Posted_by_image" => $user_image,
	"property_image" => $postImgaes,
	);
	}
...
//http://team3.nationalitsolution.co.in/estejmam/webservice/service.php?auth=fcea920f7412b5da7be0cf42b8c93759&action=propertyDetails&property_id=7

	function _propertyDetails() {
	$data = array();
	$postImgaes = array();
	...
	$getSQL="select * from `estejmam_main_property` WHERE `id`='".$property_id."'";
	
	$property = mysql_query($getSQL);
	
	$num = mysql_num_rows($property);
	if ($num > 0) {
	$propertyDetails = mysql_fetch_array($property);
...
unset($propertyComments);	

	$comments = mysql_query("select * from `estejmam_property_review` where `prop_id`='" . $currentPostID . "'");	
	while($getPropertycomments=mysql_fetch_array($comments))
	{
		
$reviewedBy = mysql_fetch_array(mysql_query("SELECT * FROM `estejmam_user` WHERE `id`='" . $getPropertycomments['user_id'] . "'"));
	
	if ($reviewedBy['image'] != '') {
	$reviewedBy_image = SITE_URL1 . "upload/userimages/" . $reviewedBy['image'];
	} else {
	$reviewedBy_image = '';
	}
	
	$propertyComments[]= array(
	"reviewed_by"=> stripslashes($reviewedBy['fname'].' '.$reviewedBy['lname']),
	"review_image"=> stripslashes($reviewedBy_image),
	"review_title"=> stripslashes($getPropertycomments['review_title']),
	"review_desc"=> stripslashes($getPropertycomments['review_desc']),
	"email_address"=> stripslashes($getPropertycomments['email_address']),
	"arrival_date"=> stripslashes($getPropertycomments['arrival_date']),
	"living_ciy"=> stripslashes($getPropertycomments['living_ciy']),
	"recomemded"=> stripslashes($getPropertycomments['recomemded']),
	"date"=> stripslashes($getPropertycomments['date']),
	);
	...
	$propertyType=mysql_fetch_array(mysql_query("SELECT * FROM `estejmam_property_type` WHERE `id`='".$propertyDetails['id']."'"));
	
   $getCountry=mysql_fetch_array(mysql_query("SELECT * FROM `estejmam_countries` WHERE `id`='".$propertyDetails['country']."'"));
	
	
	if(count($propertyComments)==0){
	$propertyComments=array();	
	}
	...
	"prop_type" => $propertyType['name'],
	"room_type" => $propertyDetails['room_type'],
	...
	"home_safety" => $propertyDetails['home_safety'],
	"price" => $propertyDetails['price'],
	"currency" => $propertyDetails['currency'],
	"lat" => $propertyDetails['lat'],
	"lng" => $propertyDetails['lng'],
	"booking_start_date" => $propertyDetails['booking_start_date'],
	"booking_end_date" => $propertyDetails['booking_end_date'],
	"posted_by" => $userDetails['fname'],
	"Posted_by_image" => $user_image,
	"property_image" => $postImgaes,
	"property_reviews" => $propertyComments,
	);
...
//http://team3.nationalitsolution.co.in/estejmam/webservice/service.php?auth=fcea920f7412b5da7be0cf42b8c93759&action=sendEmailVerification&email=nits.avik@gmail.com&userID=198
	...
	$random_hash = rand(1111,9999);
	mysql_query("UPDATE `estejmam_user` SET `email`='".$email."',`email_confirm_code`='".$random_hash."' WHERE `id`='".$userid."'");
...
//http://team3.nationalitsolution.co.in/estejmam/webservice/service.php?auth=fcea920f7412b5da7be0cf42b8c93759&action=checkEmailVerification&code=1234&userID=198
	function _checkEmailVerification(){
	$data = array();
	...
	if($code == $user_details->email_confirm_code){ 
	
	mysql_query("UPDATE `estejmam_user` SET `email_confirmed`='1' WHERE `id`='".$userid."'");
	$data['Ack'] = 1;
	$data['msg'] = 'Mail Confirmed Successfully';
	}
	else
	{
	$data['Ack'] = 0;
	$data['msg'] = 'Code Doesnot Match';
	}
	echo json_encode($data);
	return json_encode($data);		
	}
...
//http://team3.nationalitsolution.co.in/estejmam/webservice/service.php?auth=fcea920f7412b5da7be0cf42b8c93759&action=phoneVerification&phone=1234&userID=198
function _phoneVerification() {
	...
	$random_hash = rand(1111,9999);
	mysql_query("UPDATE `estejmam_user` SET `phone`='".$phone."',`opt`='".$random_hash."' WHERE `id`='".$userid."'");
	
	$data['Ack'] = 1;
	$data['msg'] = 'Sms Send Successfully';
...
//http://team3.nationalitsolution.co.in/estejmam/webservice/service.php?auth=fcea920f7412b5da7be0cf42b8c93759&action=checkPhoneVerification&code=1234&userID=198
	function _checkPhoneVerification(){
	$data = array();
	$code = isset($_POST['code']) ? $_POST['code'] : '';
	$userid = isset($_REQUEST['userID']) ? $_REQUEST['userID'] : '';
	...
	if($code == $user_details->opt){ 
	
	mysql_query("UPDATE `estejmam_user` SET `mobile_verifyed`='1' WHERE `id`='".$userid."'");
	$data['Ack'] = 1;
	$data['msg'] = 'Phone Confirmed Successfully';
	}
	else
	{
	$data['Ack'] = 0;
