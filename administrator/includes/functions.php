...
{
	$tot=mysql_num_rows(mysql_query("select * from ".$table." where ".$field."='".$id."'"));
	return $tot;
}
function updatecount($id)
{
	mysql_query("update book set click=click+1 where id='".$id."'")or die(mysql_error());
}
function thumb($filename,$width,$height,$auto,$savepath)
{
 ...
 function tep_db_perform($table, $data, $action = 'insert', $parameters = '', $link = 'sexsearch') {
  ...
  {
  	$Profile=mysql_query("select * from sex_tbluser where user_id='".$id."'")or die(mysql_error());
	if(mysql_num_rows($Profile)>0)
	{
		$RsProfile=mysql_fetch_object($Profile);
  ...
  function showmailprofile($id)
  {
	...
	if(mysql_num_rows($Profile)>0)
	{
		...
		if($RsProfile->country!="")
		{
			...
			$Country=mysql_query("select * from countries where countries_id='".$RsProfile->country."'")or die(mysql_error());
			if(mysql_num_rows($Country)>0)
			{
				$RsCountry=mysql_fetch_object($Country);
  ...
  function trace($id)
  {
   ...
   $x=@mysql_result(mysql_query("select count(*) from  sex_tblmail WHERE (to_deleted='".$id."' or from_deleted='".$id."') and deletedby <> '".$id."' "),0,0); 
	
  	return $x;
  }
  function newmail($id)
  {
  	$selectsql=mysql_query("SELECT * FROM sex_tblmail a WHERE to_id='".$id."' and to_deleted <>'".$_SESSION['userid']."' and is_new='0' and is_sent='1' and to_folderid='1' and status='1'")or die(mysql_error());
  	
	return mysql_num_rows($selectsql);
  }
  function draft($id)
  {
  	$selectsql=mysql_query("select * from sex_tblmail where from_id='".$id."' and from_deleted <>'".$_SESSION['userid']."'  and from_folderid='2' ")or die(mysql_error());
	return mysql_num_rows($selectsql);
  }
  function total($id)
  {
  	$selectsql=mysql_query("select * from sex_tblmail where status='1' and  to_id='".$id."' and is_draft='0'")or die(mysql_error());
	return mysql_num_rows($selectsql);
  }
  function send($id)
  {
  	$selectsql=mysql_query("select * from sex_tblmail a where a.from_folderid='3' and a.from_deleted <> '".$id."' and from_id='".$id."'")or die(mysql_error());
	return mysql_num_rows($selectsql);
  }
  function composeto($id)
  {
  	$selectsql=mysql_query("select * from sex_tblfriend st,sex_tbluser su where st.friend_id=su.user_id and st.friend_id<>'".$id."'")or die(mysql_error());
	return $selectsql;
  }
  function online($id)
  {
  	$Online=mysql_query("select * from sex_tbluser where online='1' and user_id<>'".$id."'")or die(mysql_error());
	$total=mysql_num_rows($Online);
...
function dateofjoin($id)
{
	...
	$sql=mysql_query("select * from sex_tbluser where user_id='". $id."'");
	if(mysql_num_rows($sql)>0)
	{
		$RsSql=mysql_fetch_object($sql);
...
function userphoto($id)
{
	...
	$Photo=mysql_query("select * from sex_tbluserphoto where user_id='".$id."' and status='1'")or die(mysql_error());
		
		if(mysql_num_rows($Photo)>0)
		{
		
			$RsPhoto=mysql_fetch_object($Photo);
...
{ 
	...
	//exit();
	$Photo=mysql_query("select * from trainee_profile where profile_id='".$id."'")or die(mysql_error());
	//$gender=@mysql_result(mysql_query("select gender from ep_user_photo where userid='".$id."'"),0,0);
...
{
	$Member=mysql_query("select * from sex_tblmail");
	return mysql_num_rows($Member);
}
function newmailimage($id)
  {
  	$selectsql=mysql_query("SELECT * FROM sex_tblmail a WHERE mail_id='".$id."' and is_new='1' and to_folderid='1' and status='1'")or die(mysql_error());
...
function sitecontent($str)
		...
		$pagedetail=@mysql_result(mysql_query("SELECT pagedetail FROM travel_cms where id='$str'"),0,0);
...
$content=mysql_fetch_array(mysql_query("select * from `frame_cms` where `id`=$id"));
