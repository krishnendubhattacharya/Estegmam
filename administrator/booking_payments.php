...
$url=basename(__FILE__)."?".(isset($_SERVER['QUERY_STRING'])?$_SERVER['QUERY_STRING']:'cc=cc');if(isset($_GET['action']) && $_GET['action']=='delete')
{
	...
	$item_id=$_GET['cid'];
	mysql_query("delete from  estejmam_property_booking where id='".$item_id."'");
...
{
	 $item_id=$_GET['cid'];
	 mysql_query("update estejmam_property_booking set is_active ='0' where id='".$item_id."'");
...
if(isset($_GET['action']) && $_GET['action']=='active')
{
	...
	$item_id=$_GET['cid'];
	mysql_query("update estejmam_property_booking set is_active='1' where id='".$item_id."'");
	$_SESSION['msg']=message('updated successfully',1);
	header('Location: '.basename(__FILE__));
	exit();
} //-----------------------------Data Manage----------------------------
...
//-----------------------------/Data Manage----------------------------
$sql="select * from estejmam_property_booking  where id<>''";
...
<html>
   ...
   function del(aa,bb)
   {
      ...
      if (a)
      {
        location.href="booking_payments.php?cid="+ aa +"&action=delete"
                  ...
                  <div class="row-fluid">
                        ...
                        <div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left">List Membership Payments</div>
                            </div>
...
?>						                <tr>
                                          ...
                                          <td><?php echo stripslashes($row->booked_date);?></td>
                                          <td><button class="btn btn-mini"  onClick="javascript:del('<?php echo $row->id; ?>')">Delete</button>&nbsp;<button class="btn btn-mini" onClick="window.location.href='booking_details.php?id=<?php echo $row->id ?>&action=details'">Details</button></td>
						                </tr>
