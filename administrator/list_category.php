...
{
	$item_id=$_GET['cid'];
	mysql_query("delete from  estejmam_category where id='".$item_id."'");
...
{
	 $item_id=$_GET['cid'];
	 mysql_query("update estejmam_category set status='0' where id='".$item_id."'");
...
{
	$item_id=$_GET['cid'];
	mysql_query("update estejmam_category set status='1' where id='".$item_id."'");
	$_SESSION['msg']=message('updated successfully',1);
	header('Location: '.basename(__FILE__));
	exit();
}
...
$sql="select * from estejmam_category  where id<>''";
...
<button class="btn btn-mini"  onClick="javascript:inactive('<?php echo $row->id;?>');">Deactivate</button>
          ...
          &nbsp;<?php /*?><button class="btn btn-mini"  onClick="javascript:del('<?php echo $row->id; ?>')">Delete</button>&nbsp;<?php */?><button class="btn btn-mini" onClick="window.location.href='add_category.php?id=<?php echo $row->id ?>&action=edit'">Edit</button></td>
						                </tr>
						            <?php 
									$count++;
									}
