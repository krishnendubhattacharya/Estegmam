...
$url=basename(__FILE__)."?".(isset($_SERVER['QUERY_STRING'])?$_SERVER['QUERY_STRING']:'cc=cc');if(isset($_GET['action']) && $_GET['action']=='delete')
{
	...
	$item_id=$_GET['cid'];
	mysql_query("delete from  estejmam_user_payments where id='".$item_id."'");
...
{
	 $item_id=$_GET['cid'];
	 mysql_query("update estejmam_user_payments set is_active ='0' where id='".$item_id."'");
...
if(isset($_GET['action']) && $_GET['action']=='active')
{
	...
	$item_id=$_GET['cid'];
	mysql_query("update estejmam_user_payments set is_active='1' where id='".$item_id."'");
	$_SESSION['msg']=message('updated successfully',1);
	header('Location: '.basename(__FILE__));
	exit();
} //-----------------------------Data Manage----------------------------
...
//-----------------------------/Data Manage----------------------------
$sql="select * from estejmam_user_payments  where id<>''";
...
<html>
   ...
   function del(aa,bb)
   {
      ...
      if (a)
      {
        location.href="membership_payments.php?cid="+ aa +"&action=delete"
