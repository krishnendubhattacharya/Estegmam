...
{
	 $item_id=$_GET['cid'];
	 mysql_query("update estejmam_banner set status='0' where id='".$item_id."'");
...
{
	$item_id=$_GET['cid'];
	mysql_query("update estejmam_banner set status='1' where id='".$item_id."'");
	$_SESSION['msg']=message('updated successfully',1);
	header('Location: '.basename(__FILE__));
	exit();
}
...
?>
						                ...
						                <tr>
						                  ...
						                  <td><img src="resize.php?pic=<?php echo stripslashes($img);?>&h=80&w=80"  style="border:1px solid #666666" /></td>
                                        <?php /*?>  <td align="center"><?php echo stripslashes($row->name);?></td><?php */?>
                                          <td>
...
<button class="btn btn-mini"  onClick="javascript:inactive('<?php echo $row->id;?>');">Deactivate</button>
		  <?php }?><?php */?>&nbsp;<button class="btn btn-mini"  onClick="javascript:del('<?php echo $row->id; ?>')">Delete</button>&nbsp;<button class="btn btn-mini" onClick="window.location.href='add_banner.php?id=<?php echo $row->id ?>&action=edit'">Edit</button></td>
						                </tr>
						            <?php 
									$count++;
									}
