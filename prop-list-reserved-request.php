...
if (isset($_REQUEST['del']) && $_REQUEST['del'] != '') {
    ...
    $del = "delete from `estejmam_booking` where `id`='" . $_REQUEST['del'] . "'";
    mysql_query($del);

    $_SESSION['MSG'] = 1;
    header("location:prop-list-reserved-request.php");
    exit();
}
?>
<html lang="en">
    <head>
        ...
        <script type="text/javascript">
            function ConfirmDelete(val)
            {
                ...
                if (sss)
                {
                    document.location.href = "prop-list-reserved-request.php?del=" + val;
                }
            }
        </script>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-9">
                        <div class="row">
                            <ul class="poplist">
                                <?php
                                $prop_booking = mysql_query("select * from `estejmam_booking` where `uploder_user_id`='" . $_SESSION['userid'] . "' and `status`='1'");
                                $num = mysql_num_rows($prop_booking);
                                if ($num > 0) {
                                    while ($allprop_book = mysql_fetch_array($prop_booking)) {
                                        
									$allprop = mysql_fetch_array(mysql_query("select * from `estejmam_main_property` where `id`='" . $allprop_book['prop_id'] . "'"));	
										
									$moreimage = mysql_fetch_array(mysql_query("select * from `estejmam_image` where `prop_id`='" . $allprop_book['prop_id'] . "'"));
                                        if ($moreimage['image'] != '') {
                                            $imag = "upload/property/" . $moreimage['image'];
                                        } else {
                                            $imag = "upload/noimage.Jpeg";
                                        }
                                        $user = mysql_fetch_array(mysql_query("select * from `estejmam_user` where `id`='" . $allprop_book['user_id'] . "'"));
                                        ?>
                                        <li>
                                            ...
                                            <a href="javascript:void(0);" onclick="javascript:ConfirmDelete('<?php echo $allprop_book['id'] ?>')" class="btn btn-primary">Cancel Booking</a>
                                        </li>
